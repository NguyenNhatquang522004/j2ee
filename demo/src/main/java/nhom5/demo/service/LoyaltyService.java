...
public class LoyaltyService {
     ...
     * Tỉ lệ mặc định: lấy từ setting LOYALTY_RATIO (mặc định 1.000đ = 1 điểm).
     */
    @Transactional
    public void awardPoints(@NonNull User user, long amount, String orderCode) {
        long ratio = Long.parseLong(settingService.getSettingValue("LOYALTY_RATIO", "1000"));
        long pointsEarned = Math.floorDiv(amount, ratio);
        if (pointsEarned <= 0) return;

        log.info("Awarding {} points to user {} for order {}", pointsEarned, user.getUsername(), orderCode);

        // 1. Cập nhật điểm
        user.setLifetimePoints((user.getLifetimePoints() != null ? user.getLifetimePoints() : 0L) + pointsEarned);
        user.setAvailablePoints((user.getAvailablePoints() != null ? user.getAvailablePoints() : 0L) + pointsEarned);
        ...
        PointTransaction transaction = PointTransaction.builder()
                ...
                .build();
        pointTransactionRepository.save(Objects.requireNonNull(transaction));
        
        // 4. Phát sự kiện
        eventPublisher.publishEvent(Objects.requireNonNull(UserEvent.builder()
                .user(user)
                .type("POINTS_EARNED")
                .points(pointsEarned)
                .build()));
    ...
    private void checkAndUpgradeTier(@NonNull User user) {
        String oldTier = user.getMembershipTier();
        long lp = user.getLifetimePoints() != null ? user.getLifetimePoints() : 0L;
        ...
        if (newTier != null && !newTier.equalsIgnoreCase(oldTier)) {
