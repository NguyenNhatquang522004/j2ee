...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    Page<Product> findByIdInAndIsActiveTrue(List<Long> ids, Pageable pageable);

    @Query("SELECT p FROM Product p WHERE p.id IN :ids AND (:isActive IS NULL OR p.isActive = :isActive) " +
            "AND (:categoryId IS NULL OR p.category.id = :categoryId) " +
            "AND (:farmId IS NULL OR p.farm.id = :farmId) " +
            "AND (:minPrice IS NULL OR p.price >= :minPrice) " +
            "AND (:maxPrice IS NULL OR p.price <= :maxPrice) " +
            "AND (:isNew IS NULL OR p.isNew = :isNew) " +
            "AND (:isSale IS NULL OR (:isSale = true AND p.originalPrice > p.price))")
    Page<Product> searchProductsByIds(
