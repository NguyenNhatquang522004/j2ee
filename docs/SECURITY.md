...
- **JWT Kill Switch (v2.0)**: Mỗi người dùng có một `token_version` trong Database. Khi đăng xuất hoặc đổi mật khẩu, version này sẽ tăng lên -> Vô hiệu hóa ngay lập tức các Token cũ đang lưu hành trên mọi thiết bị. Hệ thống đã được tối ưu hóa để kiểm tra version này tại mọi yêu cầu (Request) thông qua Filter.
- **CSP (Content Security Policy)**: Triển khai chính sách bảo mật nội dung nghiêm ngặt, chặn hoàn toàn việc thực thi script từ các nguồn không xác định, ngăn chặn triệt để tấn công XSS.
...
## 🛡️ 7. Ẩn Định Danh & Chống IDOR (ID Obfuscation)
Để bảo vệ thông tin đơn hàng và ngăn chặn việc dò tìm dữ liệu trái phép (Insecure Direct Object Reference - IDOR):
- **OrderCode Identifier**: Sử dụng `orderCode` chuỗi (VD: `ORD-20260330...`) thay thế hoàn toàn cho ID số trong mọi API công khai.
- **Ownership Enforcement**: Mọi yêu cầu truy cập chi tiết đơn hàng hay tạo link thanh toán đều kiểm tra: `Current User == Order Owner`.

---

## 📦 8. Bảo Toàn Kho Hàng & Giao Dịch (Inventory Integrity)
Hệ thống triển khai cơ chế trừ kho đa tầng:
- **FEFO (First Expired, First Out)**: Tự động trừ hàng từ các lô có thời gian hết hạn gần nhất.
- **Stock Deduction Timing**:
    - **Đơn trả trước**: Trừ kho vật lý ngay khi xác nhận thanh toán (`CONFIRMED`).
    - **Đơn COD**: Trừ kho vật lý ngay khi chuyển sang trạng thái **Đang đóng gói (PACKAGING)**.

---

## 🔍 9. Nhật ký Hậu kiểm (Audit Trail) & Token Invalidation
Mọi hành động quan trọng được ghi lại tự động:
- **Chi tiết Audit**: Ghi nhận Người thực hiện, Nội dung thay đổi, Đối tượng, IP và Thời điểm trong bảng `admin_audit_logs`.
- **Security Stamp**: `token_version` giúp vô hiệu hóa Token cũ ngay khi đổi mật khẩu hoặc đăng xuất toàn cầu.
- **Deep Traceability (SLF4J)**: Sử dụng hệ thống Logging đa điểm để ghi lại dấu vết (Traceability) của 100% đơn hàng và các Callback tài chính nhạy cảm, hỗ trợ đắc lực cho công tác điều tra và khắc phục sự cố bảo mật (Forensics).

---

## 💾 10. Bảo mật Tầng Dữ liệu & Làm sạch (Data Guard & Sanitization)
- **XSS Protection**: Sử dụng thư viện Jsoup để tự động làm sạch (Sanitize) tất cả các trường dữ liệu HTML/Text gửi lên từ người dùng (Hồ sơ, Bình luận, Tin nhắn), ngăn chặn các vụ tấn công chèn mã độc.
...
- **Advanced Identity Validation (v3.3)**: 
    - **Regex Standardization**: Áp dụng bộ lọc chính xác tiền tố nhà mạng cho số di động VN và bộ ký tự Unicode (`\p{L}`) cho Họ tên để chặn hoàn toàn ký tự lạ/số.
    - **Normalization Factory**: Tự động chuyển đổi `+84` -> `0` cho Phone và ép Email về `lowercase` tại Service layer trước khi kiểm tra tính duy nhất (`UNIQUE`) trong Database.
    - **Constraint Enforcement**: Toàn bộ định danh người dùng (`Phone`, `Email`, `Username`) được ép buộc `UNIQUE` và `NOT NULL` trực tiếp tại bảng `users`.
- **Jakarta Bean Validation**: Sử dụng `@NotBlank`, `@Size`, `@Pattern` tại các DTO để chặn dữ liệu không hợp lệ ngay từ Controller.

---

## 💳 11. Bảo mật Thanh toán (Payment Security)
...
- **Ngăn chặn Rò rỉ**: Chỉ các Key được định nghĩa an toàn (như `STORE_NAME`, `CONTACT_EMAIL`) mới được phép trả về cho người dùng không đăng nhập. Các biến môi trường nhạy cảm hoặc cấu hình nội bộ luôn được bảo vệ 100%.
- **Maintenance Mode Filter**: Tích hợp bộ lọc chuyên dụng cho phép Admin kích hoạt trạng thái bảo trì toàn hệ thống thông qua cấu hình `MAINTENANCE_MODE`. Khi kích hoạt, mọi yêu cầu từ người dùng (ngoại trừ Admin) sẽ được điều hướng an toàn về trang thông báo bảo trì chuyên nghiệp.
