...
export default function AdminBatches() {
    ...
    const canManage = hasPermission('manage:batches');
    ...
    const handleSave = async (e) => {
        ...
        if (form.productionDate && form.importDate && form.importDate < form.productionDate) {
            toast.error('Ngày nhập hàng không được trước ngày sản xuất');
            return;
        }
        if (form.expiryDate && form.importDate && form.expiryDate <= form.importDate) {
                                            ...
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    ...
                                                    <div className="flex items-center gap-2 mt-0.5">
                                                        <p className="text-[10px] text-gray-400 font-black uppercase tracking-[0.15em] flex items-center gap-1.5 whitespace-nowrap">
                                                            <TagIcon className="w-3 h-3" />
                                                            Mã: <span className="text-gray-600">{b.batchCode}</span>
                                                        </p>
                                                        {((b.productionDate && b.importDate && formatDate(b.importDate) < formatDate(b.productionDate)) || 
                                                          (b.expiryDate && b.importDate && formatDate(b.expiryDate) <= formatDate(b.importDate))) && (
                                                            <span className="flex items-center gap-1 bg-red-100 text-red-600 text-[8px] px-1.5 py-0.5 rounded font-black animate-pulse" title="Logic ngày tháng không hợp lệ!">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                               ...
                               <div className="space-y-2">
                                ...
                                <div className="space-y-2 col-span-2">
                                    ...
                                    <input required type="date" 
                                        min={form.importDate ? new Date(new Date(form.importDate).getTime() + 86400000).toISOString().split('T')[0] : TODAY}
